...
export class SsoService {
  ...
  getAuthorizedFromIDPServer(): Promise<{ statusCode, message }> {
    ...
    let promise = new Promise<{ statusCode, message }>((resolve, reject) => {
      ...
      let email: String = this.oauthService.getIdentityClaims()['Username'];
      let name: String = this.oauthService.getIdentityClaims()['name'];
        ...
        this.IdprestapiService.obtainAccessToken(params)
          .then(response => {
            if (response.status >= 500) {
                console.error("error while obtaining access token from oauth server",response);
              reject({
                statusCode: response.status,
                message: 'Looks like there was an issue during authorisation. Please try again . Contact IDP support team if issue persists.'
              });
              ...
              try {
                if (response) {
                  ...
                  reject({
                    statusCode: response.status,
                    message: "Failed to login. Please contact your IDP support team"
                  });
                }
              } catch (e) {
                console.error("error while obtaining access token from oauth server",e);
                console.error("oauth response",response);
                reject({
                  statusCode: response.status,
                  message: "Failed to login. Please contact your IDP administrator"
                });
