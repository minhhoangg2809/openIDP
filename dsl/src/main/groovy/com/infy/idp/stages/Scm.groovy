        ...
        context.with {
            ...
            steps {
                if (jsonData.buildInfo.buildtool == 'siebel') {
                    ...
                    def cmd1 = 'New-Item IDP -ItemType Directory -Force \n AutomationScripts/idp_siebel_delta_script.ps1 -SourcePath $env:IDP_WS/Repo -DestinationPath $env:IDP_WS/IDP -JenkinsURL $env:JOB_URL -JenkinsBuildNumber $env:BUILD_NUMBER -ObjectType REPO -UserName idp_oss -Password idp_oss -LogFileName $env:IDP_WS/IDP/log1.log'
                    pw.add(delegate, cmd1.toString())
                    def cmd2 = 'AutomationScripts/idp_siebel_delta_script.ps1 -SourcePath $env:IDP_WS/NonRepo -DestinationPath $env:IDP_WS/IDP -JenkinsURL $env:JOB_URL -JenkinsBuildNumber $env:BUILD_NUMBER -ObjectType NONREPO -UserName idp_oss -Password idp_oss -LogFileName $env:IDP_WS/IDP/log1.log'
